...
def validate(args, trainer, epoch_itr, valid_subset, dataname):
    ...
    stats = multi_task_avg(args, stats, metric=['auroc, aurpc'])
...
def multi_task_avg(args, stats, dataname, metrics):
    ...
    #     stats[f'{dataname}_avg_auroc']=safe_round(np.mean(
    #             [stats[key] for key in stats if 'auroc' in key]), 3)
    #         logging_output[f'{dataname}_avg_auprc']=safe_round(np.mean(
    #             [logging_output[key] for key in logging_output if 'auprc' in key]), 3)
    return None
