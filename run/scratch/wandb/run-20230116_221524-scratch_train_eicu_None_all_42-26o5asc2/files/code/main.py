...
def get_parser():
    ...
    parser.add_argument( # 'loss' for pretrain, 'auFprc' for finetune #-> 각 테스크에 따라서 자동 조정
        '--best_checkpoint_metric', type=str, default='avg_auprc', choices=['loss', 'avg_auprc', 'acc', 'avg_auroc']
    )
...
def train(args, trainer, cum_data_count, epoch_itr, epoch, sampler, validation):
    ...
    stats = multi_task_avg(args, get_training_stats(metrics.get_smoothed_values('train')), args.train_src, metrics=['auroc', 'aurpc'])
...
def validate(args, trainer, epoch_itr, valid_subset, dataname):
    ...
    print('valid smoothed_values')
    breakpoint()
    stats = multi_task_avg(args, agg.get_smoothed_values(), dataname, metrics=['auroc', 'aurpc'])
