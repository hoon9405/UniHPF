...
def get_parser():
    ...
    parser.add_argument('--criterion', type=str, default='cross_entropy') #-> 각 테스크에 따라서 자동 조정
...
def pre_main(args):
    ...
    if 'pretrain' in args.train_task:
        args.checkpoint_prefix = f'{args.pretrain_task}_train_{args.train_src}_{args.time_embed}_{args.seed[0]}_'
    elif args.train_task == 'finetune':
        args.checkpoint_prefix = f'{args.pretrain_task}_pt_{args.pt_src}_train_{args.train_src}_{args.ratio}_{args.time_embed}_{args.seed[0]}_'
    elif args.train_task == 'scratch':
        args.checkpoint_prefix = f'train_{args.train_src}_{args.ratio}_{args.time_embed}_{args.seed[0]}_'
...
def main(args) -> None:
    ...
    dataset_list = list(zip(data_split, datanames))
...
def validate(args, trainer, epoch_itr, valid_subset):
    ...
    with metrics.aggregate(new_root=True) as agg:
        for i, sample in enumerate(epoch_itr[valid_subset][args.eval_src[0]]):
